...
const MainTimeline = () => {
  ...
  const fetchTimelineData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/api/timelines?page=${page}&limit=${limit}`, {
        headers: {
      ...
      if (response.data.posts.length < 5) {//posts資料已到最後
        ob = false; // 暫時停用 Intersection Observer
      }

    } catch (error) {
      console.error('獲取時間線資料時發生錯誤：', error);
        ...
        <Grid container spacing={1}>
          {posts.map((post) => (
            <Grid item key={post.id} xs={6} sm={4} md={3}>
              <Card onClick={() => handleCardClick(post.id)} sx={{ maxWidth: 345, borderRadius: '26px' }}>
                <CardActionArea>
